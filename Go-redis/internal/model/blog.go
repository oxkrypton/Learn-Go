...
type Blog struct {
	...
	ShopID     uint64    `gorm:"column:shop_id;not null" json:"shopId"`
...
type BlogComments struct {
	ID         uint64    `gorm:"primaryKey;autoIncrement;column:id" json:"id"`
	UserID     uint64    `gorm:"column:user_id;not null" json:"userId"`
	BlogID     uint64    `gorm:"column:blog_id;not null" json:"blogId"`
	ParentID   uint64    `gorm:"column:parent_id;not null;comment:一级评论id，0代表一级评论" json:"parentId"`
	AnswerID   uint64    `gorm:"column:answer_id;not null;comment:回复的评论id" json:"answerId"`
